...
`);
        ...
        expect(result.success).toBe(true);
        expect(result.bindings['weatherData']['city']).toBe('Beijing');
    }, 10_000);
});
// ── 5.2 DAG ──────────────────────────────────────────────────────────────────
...
`);
        ...
        expect(result.bindings['weatherShanghai']['city']).toBe('Shanghai');
        expect(result.bindings['weatherBeijing']['city']).toBe('Beijing');
        expect(result.bindings['weatherGuangzhou']['city']).toBe('Guangzhou');
...
`);
        ...
        expect(result.bindings['result']['postUrl']).toBe('https://xhs.com/post/123');
    }, 10_000);
...
`);
        ...
        expect(result.stepOutputs['route']['next']).toBe('review-log');
...
`);
        ...
        expect(result.success).toBe(true);
        const translations = result.bindings['translations'];
...
`);
        ...
        expect(result.bindings['priceData']['price']).toBe(80);
    }, 10_000);
...
`);
        ...
        expect(result.success).toBe(true);
        expect(result.bindings['priceData']['price']).toBe(50);
    }, 10_000);
});
//# sourceMappingURL=chapter5-examples.test.js.map