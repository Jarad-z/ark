...
export class AutoModeOrchestrator {
     ...
     */
    shouldFireBefore(plan, stepId) {
        return plan.autoMode?.decisionStep.before === stepId;
    }
    async runDecision(plan, ctx) {
        const decisionStep = plan.autoMode?.decisionStep;
        if (!decisionStep)
            return;
        process.stderr.write('[ark:runtime] [auto] Running AI decision step...\n');
        const result = await this.bridge.makeRuntimeDecision(decisionStep.prompt, { flags: ctx.flags, mode: ctx.mode });
...
//# sourceMappingURL=auto-mode-orchestrator.js.map