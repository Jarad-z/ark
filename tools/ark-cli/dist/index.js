...
async function main() {
    switch (command) {
        ...
        case 'describe': {
            const id = rest[0];
            if (!id) {
                process.stderr.write('Usage: ark describe <id>\n');
                process.exit(1);
            }
        ...
        case 'validate': {
            const id = rest[0];
            if (!id) {
                process.stderr.write('Usage: ark validate <id>\n');
                process.exit(1);
            }
            process.stdout.write(`\nValidating ${id}...\n`);
        ...
        case 'lineage': {
            const id = rest[0];
            if (!id) {
                process.stderr.write('Usage: ark lineage <id>\n');
                process.exit(1);
            }
        ...
        case 'run': {
            const id = rest[0];
            if (!id) {
                process.stderr.write('Usage: ark run <id> [flags...]\n');
                process.exit(1);
            }
        ...
        case 'wrap': {
            ...
            if (!cli || !cmd || !id || !out) {
                process.stderr.write('Usage: ark wrap --cli <binary> --cmd "<subcommand>" --id "<pkg-id>" --out <dir>\n' +
