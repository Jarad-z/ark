...
export function showLineage(id: string, monorepoRoot: string): void {
  const tree = buildTree(id, monorepoRoot, 0, new Set())
  process.stdout.write('\nLineage tree for ' + id + '\n\n')
  process.stdout.write(tree + '\n')
}

function buildTree(id: string, monorepoRoot: string, depth: number, visited: Set<string>): string {
  if (visited.has(id)) return indent(depth) + id + ' (circular)\n'
  visited.add(id)
