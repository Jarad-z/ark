...
describe('PipelineRunner — DAG', () => {
  ...
  it('respects dependsOn: c runs only after a and b complete', async () => {
     ...
     */
    for (const [name, value] of [['cli-dep-a', 'AAA'], ['cli-dep-b', 'BBB']] as [string, string][]) {
      await writeFakeCli(
        tmpDir,
        name,
        `process.stdout.write('ARK_OUTPUT:${JSON.stringify({ result: value })}\\n')\n`
      )
    }
...
`)
  ...
  it('streaming downstream failure propagates as success: false', async () => {
    ...
    await writeFakeCli(
      tmpDir,
      'cli-single-event',
      `process.stdout.write('ARK_OUTPUT:{"value":1}\\n')\nprocess.exit(0)\n`
    )
